...
def get_pie_chart(entered_site, payload_slider):
    ...
    if entered_site == 'ALL':
        ...
        nl = [filtered_df[filtered_df['class']==0].shape[0], filtered_df[filtered_df['class']==1].shape[0]]
        cl = [0,1]
        df_temp = pd.DataFrame({'class': cl, 'num_launches': nl})
        fig = px.pie(df_temp, values='num_launches', 
        names='class', 
